...
outputCSVDirectory <- file.path(dirname(inputMS2), paste("ExclusionList_injection", polarity,"_",runNumber, ".csv", sep = ""))
...
# if using the new MSConvert get rid of the extra line
if(length(which(MSMS=="NativeID"))>0) {
MSMS<-MSMS[-c((which(MSMS=="NativeID")-1),which(MSMS=="NativeID"),which(MSMS=="NativeID")+1,which(MSMS=="NativeID")+2,which(MSMS=="NativeID")+3)]
}
